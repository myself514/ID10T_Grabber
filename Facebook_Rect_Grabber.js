...
async function generateImage(users) {
    const cols = Math.max(4, Math.ceil(Math.sqrt(users.length) / 2)); 
    const colW = 320; 
    const rowH = 80; 
    const pad = 40; 
    const size = 60;
    ...
    const defaultTitle = "These are the " + users.length + " village 1d10ts";
    let customTitle = prompt("Enter the title for the image:", defaultTitle);
    if (customTitle === null || customTitle.trim() === "") customTitle = defaultTitle;

    ctx.fillText(selectedReaction + customTitle, cw / 2, pad + 40);
    ...
    ctx.fillText("Generated by Facebook Reaction Grabber", pad, ch - 20);
